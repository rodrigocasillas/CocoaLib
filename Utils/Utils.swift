...
class Utils: UIViewController {
    ...
    // decodable from ActivateJSON for some reason can't parse JSON,
    ...
    static func checkSuccess(data: Data) {
        do {
            if let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any] {
                ...
                if !success {
                    DispatchQueue.main.async {
                        self.toastMessage(COULD_NOT_ACTIVATE + hint, color: #colorLiteral(red: 0.9254902005, green: 0.2352941185, blue: 0.1019607857, alpha: 1))
                        return
                    }
                }
            }
